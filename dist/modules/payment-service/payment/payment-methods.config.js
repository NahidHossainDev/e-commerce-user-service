"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.AVAILABLE_PAYMENT_METHODS = void 0;
const interface_1 = require("../../../common/interface");
exports.AVAILABLE_PAYMENT_METHODS = [
    {
        category: 'mobile_wallet',
        label: 'Mobile Wallet',
        methods: [
            {
                code: interface_1.PaymentMethod.BKASH,
                label: 'bKash',
                icon: 'https://cdn.yoursite.com/payments/bkash.svg',
                redirectUrl: '/pay/bkash',
                minAmount: 10,
                maxAmount: 25000,
                fee: 0,
                enabled: true,
            },
            {
                code: interface_1.PaymentMethod.NAGAD,
                label: 'Nagad',
                icon: 'https://cdn.yoursite.com/payments/nagad.svg',
                redirectUrl: '/pay/nagad',
                minAmount: 10,
                maxAmount: 20000,
                fee: 0,
                enabled: true,
            },
            {
                code: interface_1.PaymentMethod.ROCKET,
                label: 'Rocket',
                icon: 'https://cdn.yoursite.com/payments/rocket.svg',
                redirectUrl: '/pay/rocket',
                minAmount: 10,
                maxAmount: 20000,
                fee: 0,
                enabled: true,
            },
        ],
    },
    {
        category: 'card',
        label: 'Card / Net Banking',
        methods: [
            {
                code: interface_1.PaymentMethod.SSL,
                label: 'Visa / MasterCard / DBBL / AMEX (SSLCommerz)',
                icon: 'https://cdn.yoursite.com/payments/card.svg',
                redirectUrl: '/pay/ssl',
                minAmount: 50,
                fee: 2.5,
                enabled: true,
            },
        ],
    },
    {
        category: 'cod',
        label: 'Cash on Delivery',
        methods: [
            {
                code: interface_1.PaymentMethod.COD,
                label: 'Cash on Delivery',
                icon: 'https://cdn.yoursite.com/payments/cod.svg',
                redirectUrl: null,
                maxAmount: 5000,
                enabled: true,
            },
        ],
    },
];
//# sourceMappingURL=payment-methods.config.js.map